...
import io
from aws_config import upload_file_to_s3, delete_local_file
import tempfile
import shutil
...
def analyze_marks(file_path, report_details):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        total_marks_dir = os.path.join(output_dir, "total_marks")
        ...
        # Array to store S3 URLs of uploaded files
        s3_urls = []
        generated_files = []
        ...
        try:
            ...
            # Upload each generated file to S3 and get URLs
            for local_file in generated_files:
                try:
                    s3_url = upload_file_to_s3(local_file, report_details)
                    s3_urls.append(s3_url)
                except Exception as e:
                    print(f"Error uploading file {local_file} to S3: {str(e)}")
                    continue

            return final_df, s3_urls
